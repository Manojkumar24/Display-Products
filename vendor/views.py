...
def add_existing_prod(request, cat_id, p_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            v = VendorQty.objects.create(Vendor=request.user, product=p)
            # v.product = p
            v.qty = quantity
            v.save()
            return redirect('vendor:view_products')
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if Product.objects.filter(prod_name=name).exists():
                ...
                product = form.save()
                VendorQty.objects.create(Vendor=request.user,product=product, qty=form.cleaned_data['quantity'])
                return redirect('vendor:view_products')
        else:
...
def view_products(request):
    vendor_list = VendorQty.objects.filter(Vendor=request.user)
    vendor_product_list = []
    for row in vendor_list:
        vendor_product_list.append(row.product.prod_name)
    product_list = Product.objects.filter(prod_name__in=vendor_product_list)
    current_order_products = []
